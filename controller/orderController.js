...
const createOrder = async (req, res) => {
    ...
    if (!shipping_address_id) return res.status(400).json({ message: "Vui lòng chọn địa chỉ giao hàng." });
    if (!orderItemsInput || !Array.isArray(orderItemsInput) || orderItemsInput.length === 0) return res.status(400).json({ message: "Giỏ hàng trống hoặc định dạng mặt hàng không hợp lệ." });
    for (const item of orderItemsInput) {
         if (!item.item_id || !item.quantity || typeof item.quantity !== 'number' || item.quantity <= 0 || !Number.isInteger(item.quantity)) {
              return res.status(400).json({ message: `Thông tin mặt hàng không hợp lệ (ID: ${item.item_id}, Số lượng: ${item.quantity}). Số lượng phải là số nguyên dương.` });
         }
    }
    const validPaymentMethods = ['cod', 'credit_card', 'e_wallet', 'bank_transfer']; // Lấy từ ENUM hoặc config
    if (!validPaymentMethods.includes(payment_method)) return res.status(400).json({ message: "Phương thức thanh toán không hợp lệ." });
    ...
    try {
        ...
        for (const inputItem of orderItemsInput) {
            ...
            if (!dbItem.product?.shop?.shop_id) {
                await transaction.rollback();
                return res.status(500).json({ message: `Lỗi xác định cửa hàng cho mặt hàng ID ${dbItem.item_id}. Vui lòng liên hệ hỗ trợ.`});
            }
            ...
            if (shopId === null) {
                ...
                await transaction.rollback();
                return res.status(400).json({ message: "Hiện tại hệ thống chỉ hỗ trợ đặt hàng từ một cửa hàng duy nhất trong một lần thanh toán." });
            }

            // Kiểm tra tồn kho
            if (dbItem.stock < inputItem.quantity) {
                await transaction.rollback();
                return res.status(400).json({ message: `Số lượng tồn kho không đủ cho sản phẩm '${dbItem.product.title}' (ID mặt hàng: ${dbItem.item_id}). Chỉ còn ${dbItem.stock}.` });
            }
        ...
        if (shopId === null) {
             await transaction.rollback();
             return res.status(500).json({ message: "Không thể xác định cửa hàng cho đơn hàng." });
        }
        ...
        if (shipping_method_id) {
            ...
            if (selectedShippingMethod) {
                 ...
                 console.warn(`Không tìm thấy hoặc không hoạt động phương thức vận chuyển ID ${shipping_method_id}. Áp dụng phí mặc định.`);
            ...
            shippingFee = totalSubtotal > 500000 ? 0 : 30000; // Ví dụ logic mặc định
            console.log("Không có shipping_method_id, áp dụng phí mặc định:", shippingFee);
        }
        ...
        if (selectedShippingMethod) { // Chỉ tạo nếu có phương thức VC hợp lệ được chọn và tìm thấy
            ...
            // hoặc bỏ qua nếu không có method cụ thể được chọn.
             console.log(`Order ${newOrder.order_id}: Không tạo OrderShipping vì không có shipping_method_id hợp lệ.`);
        ...
        const initialPaymentStatus = (payment_method === 'cod') ? 'pending' : 'pending'; // Hoặc 'unpaid' tùy logic
        ...
        if (clear_cart === true) {
            ...
            try {
                 ...
                 console.log(`Đã xóa ${cartClearedCount} mặt hàng khỏi giỏ hàng của người dùng ${userId}.`);
            } catch (cartError) {
        ...
        const createdOrderDetails = await Order.findByPk(newOrder.order_id, {
             include: [
                  ...
                  { model: OrderShipping, as: 'orderShipping', include: [
         ...
         if (!createdOrderDetails) {
             ...
             console.error(`Không tìm thấy đơn hàng ${newOrder.order_id} sau khi tạo.`);
             // Trả về thông tin cơ bản nếu không lấy được chi tiết
             return res.status(201).json({ order_id: newOrder.order_id, order_code: newOrder.order_code, message: "Đơn hàng đã được tạo thành công.", cartClearedCount: cartClearedCount });
         }
        ...
        if (error.name === 'SequelizeForeignKeyConstraintError') {
            return res.status(400).json({ message: "Thông tin liên kết không hợp lệ (ví dụ: shop_id, user_id không tồn tại)." });
        }
        res.status(500).json({ message: error.message || "Đã xảy ra lỗi máy chủ khi tạo đơn hàng. Vui lòng thử lại." });
    }
};
...
const getUserOrders = async (req, res) => {
  ...
  try {
      const data = await Order.findAndCountAll({
          ...
          include: [ // Include basic info, avoid too many details for list view
              { model: Shop, as: 'shop', attributes: ['shop_id', 'shop_name', 'logo_url'] },
               // Maybe include just the count of items or first item image?
              {
                 model: OrderItem,
                 as: 'orderItems',
                 attributes: ['order_item_id', 'item_name', 'item_image_url', 'quantity'], // Select fewer fields
...
const getOrderDetails = async (req, res) => {
  ...
  try {
      ...
      if (!order) {
          return res.status(404).send({ message: `Không tìm thấy đơn hàng với ID=${orderId}.` });
      }
      ...
      // 3. Hoặc là shop nhận order
       const isSellerOfOrder = userRole === 'seller' && order.shop?.owner_id === userId; // Check if shop info is included

      if (!isOrderOwner && !isAdmin && !isSellerOfOrder) {
           return res.status(403).send({ message: "Bạn không có quyền xem chi tiết đơn hàng này." });
      }
      ...
      res.status(500).send({ message: "Đã xảy ra lỗi khi lấy chi tiết đơn hàng." });
  }
};
...
const updateOrderStatus = async (req, res) => {
  ...
  const validStatuses = Order.getAttributes().status.values;
   if (!validStatuses || !validStatuses.includes(newStatus)) {
      return res.status(400).json({ message: `Trạng thái '${newStatus}' không hợp lệ.` });
  }
  ...
  try {
      ...
      if (!order) {
       ...
       const isAdmin = userRole === 'admin';
       const isSellerOfOrder = userRole === 'seller' && order.shop?.owner_id === userId;

      if (!isAdmin && !isSellerOfOrder) {
           await transaction.rollback();
           return res.status(403).send({ message: "Bạn không có quyền cập nhật trạng thái đơn hàng này." });
      }
      ...
      if (!allowedTransitions[currentStatus]?.includes(newStatus)) {
           await transaction.rollback();
          return res.status(400).send({ message: `Không thể chuyển trạng thái từ '${currentStatus}' sang '${newStatus}'.` });
      }
      ...
      if (newStatus === 'shipped' || newStatus === 'delivered' || newStatus === 'canceled') {
          ...
          if (orderShipping) {
           ...
           } else if (newStatus !== 'canceled') { // Don't warn if canceling an order without shipping info
               console.warn(`OrderShipping record not found for order ${orderId} when updating status to ${newStatus}.`);
          }
      }
      ...
      res.status(500).send({ message: "Đã xảy ra lỗi khi cập nhật trạng thái đơn hàng." });
  }
}
